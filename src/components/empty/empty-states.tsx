...
export function NoMatrixTasksEmpty({ onVisitTasks }: { onVisitTasks?: () => void }) {
  return (
    <EmptyState
      icon={<IconGridDots size={32} />}
      title="No tasks to categorize"
      description="Complete some time tracking sessions first, then return here to categorize your tasks using the Eisenhower Matrix."
      actionLabel="Track Tasks"
      onAction={onVisitTasks}
    />
  )
}
